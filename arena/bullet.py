...
class Bullet(Entity):
    ...
    性能优化：每N帧做一次raycast而非每帧，中间帧用位移检测
    """
    ...
    def update(self):
        Bullet._global_frame += 1

        move_distance = self.speed * time.dt * Config.BULLET_SPEED_MULTIPLIER
        self._distance_traveled += move_distance

        if self._distance_traveled > self.max_distance:
            self._remove_from_list()
            destroy(self)
            return

        # 每2帧做一次raycast，减少碰撞检测开销
        # 中间帧只做位移，raycast的距离加倍补偿
        do_raycast = (Bullet._global_frame % 2 == 0)

        if do_raycast:
            ray_dist = move_distance * 2.0  # 补偿跳过的帧
            hit_info = raycast(
                self.position,
                self.direction,
                distance=ray_dist,
            ...
            if hit_info.hit:
                ...
                # 友军/自己/无敌：子弹穿过，不销毁
                    ...
                    target.animate_color(original_color, duration=0.1)
                    self.on_hit(hit_info, hit_player=False, hit_goal=False)
                    self._remove_from_list()
                    destroy(self)
                    return
